...
import { createHiloAction } from "@/app/tutores/materias/actions";
...
export default async function TutorMateriaForoPage({
  params,
}: PageProps<"/tutores/materias/[grupoMateriaId]/foro">) {
  ...
  const hilos = await getHilosParaGrupoMateria(admin, curso.grupoId, curso.materiaId);
  const base = `/tutores/materias/${grupoMateriaId}/foro`;
      ...
      {hilos.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center py-8 text-center text-sm text-muted-foreground">
            ...
            Todavía no hay hilos en el foro — anima a tus estudiantes a preguntar aquí.
          </CardContent>
        </Card>
      ) : (
